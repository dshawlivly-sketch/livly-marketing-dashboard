...
const PRIORITY_ORDER = { Critical: 0, 'High Priority': 1, Low: 2, Done: 3 }
const PRIORITY_CFG = {
  ...
  Done:          { color: B.green,   bg: 'rgba(90,191,130,0.14)', label: 'Done' },
}
...
function isCompletedThisWeek(task) {
  ...
  // Treat as completed this week if no dateAdded or dateAdded within last 7 days
  if (!task.dateAdded) return true
  const d = new Date(task.dateAdded)
...
function TaskCard({ task, onStatusChange, compact = false }) {
  ...
  return (
    <div style={{ background: 'rgba(255,255,255,0.04)', border: `1px solid ${overdue ? 'rgba(224,90,74,0.3)' : B.border}`, borderRadius: 7, padding: compact ? '8px 10px' : '10px 12px', marginBottom: 6 }}>
      <div style={{ display: 'flex', alignItems: 'flex-start', gap: 8, marginBottom: priCfg || task.dueDate ? 7 : 0 }}>
      ...
      {(priCfg || task.dueDate || task.notes) && (
        <div style={{ display: 'flex', gap: 6, alignItems: 'center', flexWrap: 'wrap', marginLeft: 2 }}>
          {priCfg && (
            <span style={{ background: priCfg.bg, color: priCfg.color, fontSize: 9, fontWeight: 600, padding: '2px 6px', borderRadius: 3, letterSpacing: '0.05em' }}>
              {priCfg.label}
            </span>
          )}
          {task.dueDate && (
    ...
    <div style={{ background: B.surface, border: `1px solid ${color}30`, borderRadius: 10, display: 'flex', flexDirection: 'column', overflow: 'hidden', minHeight: 260 }}>
      <div style={{ padding: '12px 14px 10px', borderBottom: `1px solid ${B.border}`, display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexShrink: 0 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 7 }}>
          ...
          <span style={{ fontSize: 11, fontWeight: 600, color: color, letterSpacing: '0.06em', textTransform: 'uppercase' }}>{title}</span>
        </div>
        {count > 0 && (
          <span style={{ background: `${color}20`, color: color, fontSize: 10, fontWeight: 600, padding: '2px 7px', borderRadius: 10 }}>{count}</span>
        )}
      </div>
...
export default function ActionCenter() {
    ...
    try {
      ...
      if (data.tasks) {
        ...
        if (data.tasks.length > prevCount) {
          const newOnes = data.tasks.slice(0, data.tasks.length - prevCount)
          newOnes.forEach(t => notifyNewTask(t.task))
        }
      }
  ...
  const thisWeek = tasks
    .filter(t => isThisWeek(t.dueDate) && t.status !== 'Done')
  ...
  // Expose completed tasks for weekly recap
  useEffect(() => {
    localStorage.setItem('livly-recap-tasks', JSON.stringify(completedThisWeek))
  }, [completedThisWeek])
    ...
    <div style={{ padding: 24 }}>
      ...
      <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: 20, flexWrap: 'wrap', gap: 12 }}>
        <div>
          ...
          <div style={{ fontSize: 13, color: B.textSec }}>
            {tasks.length} tasks · {completedThisWeek.length} done · {blocked.length > 0 && <span style={{ color: '#e05a4a' }}>{blocked.length} blocked · </span>}
      ...
      {tasks.length === 0 && !loading && (
        <div style={{ background: B.surface, border: `1px solid ${B.border}`, borderRadius: 10, padding: 40, textAlign: 'center', marginBottom: 20 }}>
          ...
          <div style={{ fontSize: 12, color: B.textTert }}>Press "Sync from Notion" or use the global Pull button in the sidebar to load your tasks.</div>
        </div>
      )}
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 14 }}>
        ...
        <BentoCell title="High Priority" color={B.coral} count={highPriority.length} emptyMsg="No critical or high-priority tasks">
          {highPriority.map(t => <TaskCard key={t.id} task={t} onStatusChange={handleStatusChange} />)}
        </BentoCell>

        {/* This Week */}
        <BentoCell title="Due This Week" color={B.amber} count={thisWeek.length} emptyMsg="Nothing due this week">
          {thisWeek.map(t => <TaskCard key={t.id} task={t} onStatusChange={handleStatusChange} />)}
        </BentoCell>

        {/* In Progress */}
        <BentoCell title="In Progress" color={B.blue} count={inProgress.length + blocked.length} emptyMsg="Nothing in progress">
          {blocked.map(t => <TaskCard key={t.id} task={t} onStatusChange={handleStatusChange} compact />)}
          {inProgress.map(t => <TaskCard key={t.id} task={t} onStatusChange={handleStatusChange} compact />)}
        </BentoCell>

        {/* Completed This Week */}
        <BentoCell title="Completed" color={B.green} count={completedThisWeek.length} emptyMsg="Nothing completed yet — get after it">
          {completedThisWeek.map(t => <TaskCard key={t.id} task={t} onStatusChange={handleStatusChange} compact />)}
          {completedThisWeek.length > 0 && (
            <div style={{ fontSize: 10, color: B.textTert, marginTop: 8, fontStyle: 'italic' }}>
              ✓ Mapped to weekly recap
            </div>
          )}
        </BentoCell>
      </div>
